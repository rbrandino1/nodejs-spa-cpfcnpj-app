...
const { buildMongoQuery, createDocumentsStream, getMongoDocumentsCollection, getMongoClient } = require('../mongo')
...
describe('getMongoDocumentsCollection', () => {
  ...
  it('returns MongoDB collection', async() => {
    ...
    expect(collection.database.name).toBe(process.env.MONGO_DATABASE_ADDRESS_TEST)
    expect(collection.database.client.databaseAddress).toBe(process.env.MONGO_DATABASE_ADDRESS_TEST)
  })

  it('throws error if collection cannot be opened', () => {
    process.env.MONGO_DATABASE_ADDRESS = null

    expect(getMongoDocumentsCollection()).rejects.toThrow()
  })
})

describe('createDocumentsStream', () => {
  const collection = new MongoCollection('documents', {})

  it('filters collection with given filters', () => {
    const filters = { type: 'cnpj' }

    const cursor = createDocumentsStream({
      collection,
      filters
    })
  ...
  it('limits query to fetch 100 records if limit parameter is absent', () => {
    const cursor = createDocumentsStream({ collection })
...
describe('buildMongoQuery', () => {
  ...
  it('parses date and use a `$gte` filter when query has `fromDate`', () => {
    expect(buildMongoQuery({ fromDate: queryParams.fromDate })).toEqual({ date: { $gte: new Date('2019-06-15') } })
  })

  it('parses date and use a `$lt` filter when query has `toDate`', () => {
    expect(buildMongoQuery({ toDate: queryParams.toDate })).toEqual({ date: { $lt: new Date('2019-06-15') } })
  })
